...
const ImageSlider = ({images, headerText, bodyText}: { images: Media[], headerText: string, bodyText: string }) => {
    ...
    return <div className="max-h-screen max-w-full relative w-full">
        {images.length === 1
            ? <div className="flex w-full bg-pale-2"><Image style={{maxWidth:`${(images[0].width||0)*1.5}px`,objectPosition: `${images[0].focalX}% ${images[0].focalY}%`}} className="max-h-[80vh] w-full object-cover grow mx-auto" src={images[0].url || ""} alt={images[0].alt || ""} width={images[0].width || 0}
