...
import qs from "qs";
import {News} from "@/app/types/payloadTypes";
...
async function getPage(query: any, tag: string, page?: string): Promise<News> {
    ...
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_PAYLOAD_SERVER_URL}/api/news/${stringifiedQuery}&depth=2`,
        {
            next: {
                tags: [tag]
            }
