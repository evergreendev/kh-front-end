...
import qs from "qs";
import {Event, Impact, MuseumCollection} from "@/app/types/payloadTypes";
...
async function getPage(query: any, tag: string, page?: string): Promise<Impact> {
    ...
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_PAYLOAD_SERVER_URL}/api/impact/${stringifiedQuery}&depth=2`,
        {
            next: {
                tags: [tag]
            }
