...
import qs from "qs";
import {Event, MuseumCollection, Passion} from "@/app/types/payloadTypes";
...
async function getPage(query: any, tag: string, page?: string): Promise<Passion> {
    ...
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_PAYLOAD_SERVER_URL}/api/studentSpotlight/${stringifiedQuery}&depth=2`,
        {
            next: {
                tags: [tag]
            }
